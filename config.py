...
def checkConfig():
	# db is a mysqlconnector to main 'pm' database
	db = dbconfig()
	cursor = db.cursor()
...
def make():
	# printc("[yellow][?] Do you want to configure new DATABASE or use the deafault pm DATABASE [/yellow]")
	# newDB = input("y/Y for new and n/N for old")
	# if op.upper() == "Y":
	...
	while 1:
		...
		if mp == getpass("Re-type: ") and mp!="" and len(mp) >= 8:
			break
		if len(mp) < 8:
			printc("[red][!] Please try again with at least 8 letters. [/yellow]")
		else:
			printc("[red][!] Please try again.[/yellow]")
	...
	# Add them to secrets side by side
...
def delete():
	...
	# This block is for cheking integrity of the user
	...
	if hashed_mp != result[0]:
		...
		printc("[red][!] Unable to delete the Configuration [RED]")
		db.close()

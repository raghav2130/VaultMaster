...
import utils.addEntry
import utils.retrieve
import utils.genRanPass
...
def main():
	if args.option in ["add","a"]:
		...
		if res is not None:
			...
			printc("[red][!] Wrong Password, Try Again [/red]")
	...
	if args.option in ["generate","g"]:
		if args.length == None:
			printc("[red][+][/red] Default length of generated password set to 12")
		password = utils.genRanPass.generatePassword(args.length)
